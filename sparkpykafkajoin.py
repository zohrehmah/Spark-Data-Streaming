...
spark = SparkSession.builder.appName("stedi-ecosystem").getOrCreate()
...
zSetDecodedEntriesStreamingDF.withColumn("customer",from_json("customer",customerJsonSchema)) \
        .select(col("customer.*")) \
...
riskScoreStreamingDF.selectExpr("CAST(customer AS STRING) AS key", "to_json(struct(*)) AS value")\
    .writeStream \
    .format("kafka") \
    .option("kafka.bootstrap.servers", "localhost:9092")\
    .option("topic", "customers-risk")\
